...
function init() {
    ...
    for (const index in halftoneSettings) {
        ...
        folder
            .addColor({ color: uniforms.color.value.getHexString(THREE.SRGBColorSpace) }, 'color')
    ...
    folder
        .addColor({ color: defaultMaterial.color.getHexString(THREE.SRGBColorSpace) }, 'color')
