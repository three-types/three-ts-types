...
async function init() {
    ...
    const prePass = pass(scene, camera).toInspector('Normal', inspectNode =>
        colorSpaceToWorking(inspectNode, THREE.SRGBColorSpace),
    );
    ...
    transparentMesh = new THREE.Mesh(
        ...
        new THREE.MeshStandardNodeMaterial({ transparent: true, opacity: params.transparentOpacity }),
    );
    ...
    gui.add(params, 'aoType', ['GTAO', 'SSAO'])
        .name('AO type')
        .onChange(() => {
    ...
    gui.add(params, 'transparentOpacity', 0, 1, 0.01)
        .name('mesh opacity')
        .onChange(value => {
