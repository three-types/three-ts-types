...
function init() {
    ...
    computeInitNode = computeInitWGSL({ writeTex: storageTexture(pingTexture), index: instanceIndex, seed }).compute(
        width * height,
    );
    ...
    computeToPong = computePingPongWGSL({ readTex: readPing, writeTex: writePong, index: instanceIndex }).compute(
        width * height,
    );
    computeToPing = computePingPongWGSL({ readTex: readPong, writeTex: writePing, index: instanceIndex }).compute(
        width * height,
    );
