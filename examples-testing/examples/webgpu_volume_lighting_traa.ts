...
const _haltonOffsets = Array.from({ length: 32 }, (_, i) => [halton(i + 1, 2), halton(i + 1, 3)]);
...
function init() {
    ...
    volumetricMaterial.offsetNode = fract(
        interleavedGradientNoise(screenCoordinate.add(temporalJitter2D.mul(100))).add(temporalOffset),
    );
