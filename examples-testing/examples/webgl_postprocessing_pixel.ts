...
function init() {
    ...
    snappingFolder.add(params, 'cameraZoomSnap').min(0).max(1).step(0.05).name('cameraZoomSnap (0 = off)');
...
function animate() {
    ...
    crystalMesh.material.emissiveIntensity =
        Math.round((Math.sin(t * 3) * 0.5 + 0.5) * (brightnessLevels - 1)) / (brightnessLevels - 1);
