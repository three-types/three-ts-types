...
function init() {
    ...
    new HDRLoader().setPath('textures/equirectangular/').load('san_giuseppe_bridge_2k.hdr', async texture => {
...
function reportMaterialXLog(sample, materialName, log) {
    ...
    console.groupCollapsed(
        `THREE.MaterialXLoader: ${errorCount} error(s), ${warningCount} warning(s) for ${label} (${sample})`,
    );
...
async function addSample(sample, path) {
    ...
    const { materials, log } = await new MaterialXLoader().setPath(path).loadAsync(sample, {
        uvSpace: 'top-left',
