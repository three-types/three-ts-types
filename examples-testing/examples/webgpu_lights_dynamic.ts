...
function init() {
    ...
    for (let i = 0; i < 100; i++) {
        ...
        mesh.position.set(Math.cos(angle) * radius, 0.7 + Math.random() * 2, Math.sin(angle) * radius);
...
function addLight() {
    ...
    light.position.set(Math.cos(angle) * radius, 1 + Math.random() * 6, Math.sin(angle) * radius);
    ...
    // Visual indicator
    const sphere = new THREE.Mesh(new THREE.SphereGeometry(0.15, 8, 8), new THREE.MeshBasicMaterial({ color: color }));
    light.add(sphere);
