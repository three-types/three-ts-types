...
async function init(forceGlobalSwap = false) {
    ...
    const nextAlgoBuffer = new THREE.StorageInstancedBufferAttribute(
        new Uint32Array(1).fill(forceGlobalSwap ? StepType.FLIP_GLOBAL : StepType.FLIP_LOCAL),
        1,
    );
    ...
    const nextBlockHeightStorage = storage(nextBlockHeightBuffer, 'uint', nextBlockHeightBuffer.count)
        .setPBO(true)
        .label('NextBlockHeight');
    const nextBlockHeightRead = storage(nextBlockHeightBuffer, 'uint', nextBlockHeightBuffer.count)
        .setPBO(true)
        .label('NextBlockHeight')
        .toReadOnly();

    const highestBlockHeightBuffer = new THREE.StorageInstancedBufferAttribute(new Uint32Array(1).fill(2), 1);
    const highestBlockHeightStorage = storage(highestBlockHeightBuffer, 'uint', highestBlockHeightBuffer.count)
        .setPBO(true)
    ...
    const randomizedElementsStorage = storage(randomizedElementsBuffer, 'uint', size)
        .setPBO(true)
    ...
    const display = Fn(() => {
        ...
        If(highlight.equal(1).and(not(nextAlgoStorage.element(0).equal(StepType.NONE))), () => {
            const boolCheck = int(
                elementIndex.modInt(nextBlockHeightRead.element(0)).lessThan(nextBlockHeightRead.element(0).div(2)),
            );
