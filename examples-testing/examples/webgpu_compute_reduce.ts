...
document.getElementById('panel-title').textContent +=
    ` (Subgroup Size: ${cssSubgroupSize}, Workgroup Size: ${cssWorkgroupSize})`;
...
for (let i = 0; i < cssWorkgroupSize; i++) {
    if (i % cssSubgroupSize === 0) {
        ...
        currentSubgroupDiv.setAttribute(
            'data-label',
            `Threads ${currentSubgroupIndex * cssSubgroupSize}-${(currentSubgroupIndex + 1) * cssSubgroupSize - 1}`,
        );
...
gui.add(leftEffectController, 'displayMode', displayModes)
...
gui.add(rightEffectController, 'displayMode', displayModes)
...
const WorkgroupReduce = workgroupReduceProps => {
    ...
    Loop(float(delta).greaterThan(1.0), () => {
        ...
        total.assign(
            select(invocationLocalIndex.lessThan(delta), subgroupSums.element(invocationLocalIndex), 0).uniformFlow(),
        );
    ...
    const fnDef = Fn(() => {
        ...
        Loop(
            ...
            () => {
                ...
                const t = subgroupAdd(
                    select(isValidSubgroupIndex, perSubgroupReductionArray.element(subgroupIndex), 0).uniformFlow(),
                ).toVar('t');
...
async function init(leftSideDisplay = true) {
    ...
    const inputVectorizedStorage = storage(inputVec4BufferAttribute, 'uvec4', vecSize)
        .setPBO(true)
    ...
    const workgroupSumsStorage = instancedArray(workgroupSumsArray, 'uint', numRows)
        .setPBO(true)
