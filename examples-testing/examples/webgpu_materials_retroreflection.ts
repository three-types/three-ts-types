...
async function init() {
    ...
    gui.add(params, 'flashLight', 0, 30, 0.1)
        .name('flash light')
...
function createCone(orangeMaterial, bandMaterial) {
    ...
    const base = new THREE.Mesh(createBaseGeometry(0.42, 0.04, 0.08), orangeMaterial);
    cone.add(base);
    ...
    for (const { center, height } of bands) {
        const bandRadiusBottom =
            radiusAtHeight(center - height * 0.5, bodyBottom, bodyHeight, bodyRadiusBottom, bodyRadiusTop) * 1.01;
        const bandRadiusTop =
            radiusAtHeight(center + height * 0.5, bodyBottom, bodyHeight, bodyRadiusBottom, bodyRadiusTop) * 1.01;
