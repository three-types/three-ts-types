...
async function init() {
    ...
    const background = screenUV.distance(0.5).mix(color(0x5d5d84), color(0x2e2e44));
    ...
    const floor = new THREE.Mesh(new THREE.CircleGeometry(58, 64).rotateX(-Math.PI / 2), floorMaterial);
    ...
    const liquid = new THREE.Mesh(new THREE.CircleGeometry(2, 48).rotateX(-Math.PI / 2), liquidMaterial);
    ...
    const ribbonMaterial = new THREE.MeshStandardNodeMaterial({
        color: 0xffcc44,
        metalness: 1,
...
function createHandleGeometry() {
    ...
    for (let i = 0; i <= divisions; i++) {
        ...
        for (let j = 0; j < 16; j++) {
            ...
            ring.push(new THREE.Vector3(point.x - radial * tangent.y, point.y + radial * tangent.x, b * Math.sin(phi)));
        }
...
function createRibbonGeometry() {
    ...
    for (let i = 0; i <= 120; i++) {
        ...
        sections.push([new THREE.Vector3(3 * sin, t * 7.5, 3 * cos), new THREE.Vector3(3 * sin, t * 7.5 + 2, 3 * cos)]);
    }
...
function createRopeGeometry(length) {
    ...
    for (let i = 0; i <= 40; i++) {
        ...
        for (let j = 0; j < 16; j++) {
            ...
            points.push(new THREE.Vector3(x - (radial * ty) / tl, y + (radial * tx) / tl, 0.08 * Math.sin(phi)));
        }
