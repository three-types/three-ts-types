...
async function init() {
    ...
    scene.backgroundNode = hue(normalWorld.y.mix(0, color(0x0066ff)).mul(0.1), time);
    ...
    const reflection = reflector({ resolutionScale: 0.5, depth: true, bounces: false }); // 0.5 is half of the rendering view
    ...
    floorMaterial.colorNode = Fn(() => {
        ...
        const reflectionFinal = mix(reflection.rgb, reflectionBlurred.rgb, reflectionMixFactor).mul(
            reflectionIntensity,
        );
    ...
    gui.add(reflection.reflector, 'resolutionScale', 0.25, 1).name('resolution scale');
