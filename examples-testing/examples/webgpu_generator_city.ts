...
async function init() {
    ...
    gui.add(parameters, 'seed', 0, 100, 1)
        .debounce(300)
        .onChange(() => {
            generateCity();
            scheduleRebake();
        });
    gui.add(parameters, 'timeOfDay', 6, 18, 0.1)
        .name('time of day')
        .onChange(() => {
            updateSun();
            scheduleRebake();
...
function updateSun() {
    ...
    const lightSpace = new THREE.Quaternion()
        .setFromRotationMatrix(new THREE.Matrix4().lookAt(sunLight.position, scene.position, THREE.Object3D.DEFAULT_UP))
        .invert();
