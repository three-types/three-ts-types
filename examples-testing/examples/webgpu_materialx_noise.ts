...
async function init() {
    ...
    const position3D = normalWorld.mul(4).add(vec3(time.mul(0.35), time.mul(0.19), time.mul(0.27)));
    ...
    const noiseExamples = [
        ...
        { label: ['Fractal', '3D'], createNode: () => mx_fractal_noise_vec3(fractal3D) },
        {
            ...
            createNode: () => mx_unifiednoise2d(int(0), position2D, vec2(1, 1), unifiedOffset2D),
        },
        {
            label: ['Unified Perlin', '3D'],
            createNode: () => unifiedNoise3D(int(0), position3D, vec3(1, 1, 1), unifiedOffset3D),
        },
        {
            label: ['Unified Cell', '2D'],
            createNode: () => mx_unifiednoise2d(int(1), position2D, vec2(1, 1), unifiedOffset2D),
        },
        {
            label: ['Unified Cell', '3D'],
            createNode: () => unifiedNoise3D(int(1), position3D, vec3(1, 1, 1), unifiedOffset3D),
        },
        {
            label: ['Unified Worley', '2D style 0'],
            createNode: () =>
                mx_unifiednoise2d(int(2), position2D, vec2(1, 1), unifiedOffset2D, 1, 0, 1, false, 1, 2, 0.5, 0),
        },
        {
            label: ['Unified Worley', '3D style 0'],
            createNode: () =>
                unifiedNoise3D(int(2), position3D, vec3(1, 1, 1), unifiedOffset3D, 1, 0, 1, false, 1, 2, 0.5, 0),
        },
        {
            label: ['Unified Worley', '2D style 1'],
            createNode: () =>
                mx_unifiednoise2d(int(2), position2D, vec2(1, 1), unifiedOffset2D, 1, 0, 1, false, 1, 2, 0.5, 1),
        },
        {
            label: ['Unified Worley', '3D style 1'],
            createNode: () =>
                unifiedNoise3D(int(2), position3D, vec3(1, 1, 1), unifiedOffset3D, 1, 0, 1, false, 1, 2, 0.5, 1),
        },
        {
            label: ['Unified Fractal', '2D'],
            createNode: () => mx_unifiednoise2d(int(3), position2D, vec2(1, 1), unifiedOffset2D, 1, 0, 1, false, 3),
        },
        {
            label: ['Unified Fractal', '3D'],
            createNode: () => unifiedNoise3D(int(3), position3D, vec3(1, 1, 1), unifiedOffset3D, 1, 0, 1, false, 3),
        },
        {
...
function createNoiseGrid(noiseExamples, geometry, font, labelMaterial) {
    ...
    const spacingX = 15.5;
    const spacingY = 18;
    const rowOffset = (Math.ceil(noiseExamples.length / columns) - 1) * spacingY * 0.5;
