...
function init() {
    ...
    const refractionUV = screenUV.add(vec2(0, waterIntensity.mul(0.1))).toInspector('Water / Refraction UV');
    ...
    waterMaterial.backdropNode = depthEffect
        .mix(viewportSharedTexture(), viewportTexture.mul(depthRefraction.mix(1, waterColor)))
        .mul(color(0xd3ebf8));
    ...
    floor.material.colorNode = causticFade.mix(material.colorNode, material.colorNode.add(causticNoise));
    ...
    scenePassColorBlurred.directionNode = waterMask
        ...
        .toInspector('Post-Processing / Blur Strength [ Depth ]', node => node.toFloat());

    const vignette = screenUV.distance(0.5).mul(1.35).clamp().oneMinus().toInspector('Post-Processing / Vignette');
