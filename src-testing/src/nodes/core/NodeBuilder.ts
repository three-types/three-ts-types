...
class NodeBuilder {
    ...
    isOpaque() {
        ...
        return (
            material.transparent === false && material.blending === NormalBlending && material.alphaToCoverage === false
        );
    }
    ...
    getNodeUniform(uniformNode, type) {
        ...
        if (node === undefined) {
            if (type === 'float' || type === 'int' || type === 'uint') node = new NumberNodeUniform(uniformNode);
                ...
                node = new Vector2NodeUniform(uniformNode);
            else if (type === 'vec3' || type === 'ivec3' || type === 'uvec3')
                node = new Vector3NodeUniform(uniformNode);
            else if (type === 'vec4' || type === 'ivec4' || type === 'uvec4')
                node = new Vector4NodeUniform(uniformNode);
