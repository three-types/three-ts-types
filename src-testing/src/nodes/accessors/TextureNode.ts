...
class TextureNode extends UniformNode {
        ...
        if (builder.isFlipY()) {
            ...
            if (this.sampler) {
                ...
                uvNode = this._flipYUniform.select(
                    uvNode.setY(int(textureSize(this, this.levelNode).y).sub(uvNode.y).sub(1)),
                    uvNode,
                );
            }
        }
    ...
    setup(builder) {
        ...
        const uvNode = Fn(() => {
            ...
            this.updateType =
                this._matrixUniform !== null || this._flipYUniform !== null
    ...
    generate(builder, output) {
        ...
        if (/^sampler/.test(output)) {
            ...
            if (propertyName === undefined) {
                const { uvNode, levelNode, biasNode, compareNode, compareStepNode, depthNode, gradNode, offsetNode } =
                    properties;
                ...
                if (compareStepSnippet !== null) {
                    snippet = step(expression(compareStepSnippet, 'float'), expression(snippet, nodeType)).build(
                        builder,
                        nodeType,
                    );
                }
            ...
            if (builder.needsToWorkingColorSpace(texture)) {
                snippet = colorSpaceToWorking(expression(snippet, nodeType), texture.colorSpace)
                    .setup(builder)
