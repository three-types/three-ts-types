...
class WebGPUBackend extends Backend {
    ...
    async initTimestampQuery(renderContext, descriptor) {
        ...
        if (!renderContextData.timeStampQuerySet) {
            ...
            const timeStampQuerySet = await this.device.createQuerySet({
                type: 'timestamp',
                count: 2,
                label: `timestamp_renderContext_${renderContext.id}`,
            });
