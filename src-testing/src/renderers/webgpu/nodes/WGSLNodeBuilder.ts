...
`),
    ...
    equals_bvec2: new CodeNode(
        'fn threejs_equals_bvec2( a : vec2f, b : vec2f ) -> vec2<bool> { return vec2<bool>( a.x == b.x, a.y == b.y ); }',
    ),
    equals_bvec3: new CodeNode(
        'fn threejs_equals_bvec3( a : vec3f, b : vec3f ) -> vec3<bool> { return vec3<bool>( a.x == b.x, a.y == b.y, a.z == b.z ); }',
    ),
...
if (/Windows/g.test(navigator.userAgent)) {
    wgslPolyfill.pow_float = new CodeNode(
        'fn threejs_pow_float( a : f32, b : f32 ) -> f32 { return select( -pow( -a, b ), pow( a, b ), a > 0.0 ); }',
    );
    wgslPolyfill.pow_vec2 = new CodeNode(
        'fn threejs_pow_vec2( a : vec2f, b : vec2f ) -> vec2f { return vec2f( threejs_pow_float( a.x, b.x ), threejs_pow_float( a.y, b.y ) ); }',
        [wgslPolyfill.pow_float],
    );
    wgslPolyfill.pow_vec3 = new CodeNode(
        'fn threejs_pow_vec3( a : vec3f, b : vec3f ) -> vec3f { return vec3f( threejs_pow_float( a.x, b.x ), threejs_pow_float( a.y, b.y ), threejs_pow_float( a.z, b.z ) ); }',
        [wgslPolyfill.pow_float],
    );
    wgslPolyfill.pow_vec4 = new CodeNode(
        'fn threejs_pow_vec4( a : vec4f, b : vec4f ) -> vec4f { return vec4f( threejs_pow_float( a.x, b.x ), threejs_pow_float( a.y, b.y ), threejs_pow_float( a.z, b.z ), threejs_pow_float( a.w, b.w ) ); }',
        [wgslPolyfill.pow_float],
    );
...
class WGSLNodeBuilder extends NodeBuilder {
    ...
    isUnfilterable(texture) {
        return (
            ...
            (texture.isDataTexture === true && texture.type === FloatType) ||
            texture.isMultisampleRenderTargetTexture === true
        );
    }
