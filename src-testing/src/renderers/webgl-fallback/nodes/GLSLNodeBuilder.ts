...
const glslPolyfills = {
    bitcast_int_uint: new CodeNode(
        /* glsl */ 'uint tsl_bitcast_int_to_uint ( int x ) { return floatBitsToUint( intBitsToFloat ( x ) ); }',
    ),
    bitcast_uint_int: new CodeNode(
        /* glsl */ 'uint tsl_bitcast_uint_to_int ( uint x ) { return floatBitsToInt( uintBitsToFloat ( x ) ); }',
    ),
};
                ...
                if (varying.needsInterpolation) {
                    if (varying.interpolationType) {
                        const interpolationType =
                            interpolationTypeMap[varying.interpolationType] || varying.interpolationType;
