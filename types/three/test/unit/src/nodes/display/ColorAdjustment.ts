...
const luminanceNode = new ShaderNode<{ color: Node<"vec3"> }, Node<"float">>(({ color }) => {
    const LUMA = vec3(0.2125, 0.7154, 0.0721);

    return dot(color, LUMA);
});

const saturationNode = new ShaderNode<{ color: Node<"vec3">; adjustment: Node<"float"> }, Node<"vec3">>(
    ({ color, adjustment }) => {
...
const vibranceNode = new ShaderNode<{ color: Node<"vec3">; adjustment: Node<"float"> }, Node<"vec3">>(
    ({ color, adjustment }) => {
...
const hueNode = new ShaderNode<{ color: Node<"vec3">; adjustment: Node<"float"> }>(({ color, adjustment }) => {
