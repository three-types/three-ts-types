...
export class LightShadow<TCamera extends Camera = Camera> {
    ...
    /**
     * Shadow map bias, how much to add or subtract from the normalized depth when deciding whether a surface is in shadow.<br> The Very tiny adjustments here (in the order of 0.0001) may help reduce artifacts in shadows
    ...
    /**
     * Setting this to values greater than 1 will blur the edges of the shadow.<br> High values will cause unwanted banding effects in the shadows - a greater {@link LightShadow.mapSize | mapSize} will allow for a higher value to be used here before these effects become visible.<br> If {@link THREE.WebGLRenderer.shadowMap.type | WebGLRenderer.shadowMap.type} is set to {@link Renderer | PCFSoftShadowMap}, radius has no effect and it is recommended to increase softness by decreasing {@link LightShadow.mapSize | mapSize} instead.<br><br> Note that this has no effect if the {@link THREE.WebGLRenderer.shadowMap.type | WebGLRenderer.shadowMap.type} is set to {@link Renderer | BasicShadowMap}.
     * @remarks Expects a `Float`
     * @defaultValue `1`
     */
    radius: number;
     ...
     * @remarks Values must be powers of 2, up to the {@link THREE.WebGLRenderer.capabilities | WebGLRenderer.capabilities}.maxTextureSize for a given device, although the width and height don't have to be the same (so, for example, (512, 1024) is valid).
