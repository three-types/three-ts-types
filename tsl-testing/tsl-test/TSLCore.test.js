...
function setProtoSwizzle(property, altA, altB) {
  ...
  Node.prototype["set" + propUpper] =
    Node.prototype["set" + altAUpper] =
    Node.prototype["set" + altBUpper] =
      function (value) {
...
const ShaderNodeProxy = function (
  ...
  function verifyParamsLimit(params) {
    ...
    if (minParams !== undefined && params.length < minParams) {
      error(
        `TSL: "${tslName}" parameter length is less than minimum required.`,
        new StackTrace(),
      );
      ...
      error(
        `TSL: "${tslName}" parameter length exceeds limit.`,
        new StackTrace(),
      );
...
class ShaderCallNodeInternal extends Node {
  ...
  call(builder) {
    ...
    if (shaderNode.layout) {
      ...
      const hasParameters = Array.isArray(rawInputs)
        ? rawInputs.length > 0
        : rawInputs !== null;

      const jsFunc = shaderNode.jsFunc;
      const outputNode =
        hasParameters || jsFunc.length > 1
          ? jsFunc(inputs, secureNodeBuilder)
          : jsFunc(secureNodeBuilder);
...
function getLayoutParameters(params) {
  ...
  const isArrayAsParameter =
    params[0] &&
    (params[0].isNode || Object.getPrototypeOf(params[0]) !== Object.prototype);
...
const ConvertType = function (type, cacheMap = null) {
  return (...params) => {
    for (const param of params) {
      if (param === undefined) {
        error(
          `TSL: Invalid parameter for the type "${type}".`,
          new StackTrace(),
        );
...
export const nodeObjectIntent = (val, altType = null) =>
  /* new */ nodeObject(val, altType).toVarIntent();
...
export const convert = (node, types) =>
  new ConvertNode(nodeObject(node), types);
export const split = (node, channels) =>
  new SplitNode(nodeObject(node), channels);
